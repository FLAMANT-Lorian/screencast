...
while ($country_row = fgetcsv($file_handle, 1000, escape: '')) { // On passe à ladeuxième ligne pour passer la ligne des en-têtes

    //Certains caractères peuvent casser l'analyse apparemment. Ce test est une petite précaution 🤞🍀
    if (count($country_row) === count($headers)) { // Vérifier qu'il y ale mêmme nombre de donnée que d'en-têtes
        ...
        $cca2s[] = $country_row[$cca2_index]; // Pousser des éléments dans un tableau
    }
}
...
User::query()->truncate();
User::create([
    'email' => 'lorian.flamant@student.hepl.be',
